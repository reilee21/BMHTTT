...
class MyMainWindow(QMainWindow):
    ...
    def setupMenuEvent(self):
        ...
        self.ui.btnMH_ThayThe.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnMH_ThayThe)
        )
        self.ui.btnMH_ChuyenVi.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnMH_ChuyenVi)
        )
        self.ui.btnMH_XOR.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnMH_XOR)
        )
        self.ui.btnMH_HienDai.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnMH_HienDai)
        )
        self.ui.btnGM_ThayThe.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnGM_ThayThe)
        )
        self.ui.btnGM_ChuyenVi.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnGM_ChuyenVi)
        )
        self.ui.btnGM_XOR.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnGM_XOR)
        )
        self.ui.btnGM_HienDai.pressed.connect(
            lambda: self.setActiveButton(self.ui.btnGM_HienDai)
        )
    ...
    def action(self, index):
        ...
        if ui == 5:
            ...
            self.ui.stackedWidget.setCurrentIndex(ui)
